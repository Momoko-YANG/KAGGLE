...
class FeatureEngineer:
    ...
    def _apply_feature_lagging(self, feats: pd.DataFrame) -> pd.DataFrame:
        """
        辅助方法：对指定特征应用滞后处理，以修正数据泄漏。
        """
        lag_amount = CONFIG.get("LEAKY_FEATURE_LAG", 0)
        if lag_amount <= 0:
            return feats  # 如果不设置lag，则不进行任何操作
        ...
        print(f"Applying lag of {lag_amount} days to {len(cols_to_lag)} leaky features...")

        # 对每个需要滞后的特征进行处理
        for col in cols_to_lag:
            new_col_name = f"{col}_lag{lag_amount}"

            # 1. 按target分组进行shift操作
            feats[new_col_name] = feats.groupby('target', sort=False)[col].shift(lag_amount)

            # 2. 从DataFrame中删除原始的、有泄漏的列
            feats = feats.drop(columns=[col])

            # 3. 更新特征名称集合 (非常重要！)
            if col in self.feature_names_:
                self.feature_names_.remove(col)
                self.feature_names_.add(new_col_name)
    ...
    def fit(self, train_df: pd.DataFrame, pairs_df: pd.DataFrame) -> pd.DataFrame:
        ...
        feats_time = self._apply_feature_lagging(feats_time)
        # --- 修改结束 ---
...
# ============================================================
# === GRU 辅助函数：序列数据构建 & 模型构建 ===
# 放置于 FeatureEngineer 之后，避免循环引用
# ============================================================
def create_sequential_dataset(df: pd.DataFrame, 
                              feature_cols: list, 
                              target_col: str, 
                              lookback_window: int = 30):
    """将表格数据转换为适用于RNN/GRU的序列格式。"""
    df = df.sort_values(['target', 'date_id'])
    df[feature_cols] = df[feature_cols].fillna(0.0)
    all_X_seq, all_y_seq, all_indices = [], [], []
    for _, group in df.groupby('target', sort=False):
        features = group[feature_cols].values
        labels = group[target_col].values
        for i in range(len(group) - lookback_window + 1):
            end_idx = i + lookback_window
            all_X_seq.append(features[i:end_idx, :])
            all_y_seq.append(labels[end_idx - 1])
            all_indices.append(group.index[end_idx - 1])
    if not all_X_seq:
        return np.array([]).reshape(0, lookback_window, len(feature_cols)), np.array([]), pd.Index([])
    return np.array(all_X_seq, dtype=np.float32), np.array(all_y_seq, dtype=np.float32), pd.Index(all_indices)


def build_gru_model(lookback_window: int, num_features: int):
    """构建一个简单的GRU模型。"""
    input_layer = Input(shape=(lookback_window, num_features))
    gru_layer = GRU(units=64, return_sequences=False)(input_layer)
...
# F) 时间CV + 稳健特征筛选 + 评估
# ============================================================
def make_time_folds(dates: pd.Series, n_folds: int = 4):
...
def add_hp_filter_features(df_long: pd.DataFrame, series_col: str = 'z') -> pd.DataFrame:
    """使用HP滤波分解序列，并添加周期和趋势作为特征"""
    print("Adding Hodrick-Prescott Filter features...")
    def apply_hp_filter(group):
        series = group[series_col]
        series_filled = series.ffill().bfill().fillna(0)
...
def add_stl_features(df_long: pd.DataFrame, series_col: str = 'z', period: int = 21) -> pd.DataFrame:
    """
...
def train_one_lag_enhanced(aligned_df: pd.DataFrame, 
    ...
    oof_r_gru = np.full(len(df), np.nan)
    ...
    for fold_idx, (tr_dates, va_dates) in enumerate(folds):
        ...
        try:
            ...
            lgbm_pipeline = make_pipeline(
                SimpleImputer(strategy='constant', fill_value=0.0),
                StandardScaler(), # 对于LGBM，StandardScaler是可选的，但保留也没问题
                lgb.LGBMClassifier(
                    objective='binary',          # 明确指定为二分类任务
                    metric='auc',                # 使用AUC作为评估指标
                    is_unbalance=True,           # 替代 'class_weight=balanced'，处理样本不均衡
                    n_estimators=500,            # 增加迭代次数，配合早停
                    learning_rate=0.05,
                    num_leaves=20,               # 使用较小的叶子数防止过拟合
                    max_depth=4,                 # 限制树的深度
            ...
            # 使用早停机制来找到最佳迭代次数
            lgbm_pipeline.fit(
                X_tr_fold, y_tr_dir_fold,
                lgbmclassifier__eval_set=[(X_va_fold, y_va_dir_fold)],
                lgbmclassifier__eval_metric='auc',
                lgbmclassifier__callbacks=[lgb.early_stopping(100, verbose=False)] # 100轮不提升就停止
            )
            # --- 主要修改区域结束 ---

            oof_p[va_mask] = lgbm_pipeline.predict_proba(X_va)[:, 1]
            models_lr.append(lgbm_pipeline) # 变量名可以不改，或者改成 models_clf
        except Exception as e:
            print(f"[WARN] LGBMClassifier training failed in fold {fold_idx}: {e}")
            continue
        ...
        # GRU
        try:
            df_tr, df_va = df[tr_mask], df[va_mask]
            X_tr_seq, y_tr_seq, _ = create_sequential_dataset(df_tr, feature_cols, 'y', LOOKBACK_WINDOW)
            X_va_seq, y_va_seq, va_indices = create_sequential_dataset(df_va, feature_cols, 'y', LOOKBACK_WINDOW)

            if X_tr_seq.shape[0] > 100 and X_va_seq.shape[0] > 0:
                print(f"    [lag={lag}, fold={fold_idx}] Training GRU model...")
                gru_model = build_gru_model(LOOKBACK_WINDOW, len(feature_cols))
                ...
                gru_preds = gru_model.predict(X_va_seq, verbose=0).flatten()
                # 将验证集序列对应的末端样本位置回填为 OOF
                tmp = pd.Series(np.nan, index=df.index)
                tmp.loc[va_indices] = gru_preds
                oof_r_gru = np.where(np.isnan(oof_r_gru), tmp.values, oof_r_gru)
                models_gru.append(gru_model)
        except Exception as e:
            print(f"[WARN] Fold {fold_idx} GRU training failed: {e}")

    # 融合策略（Stacking）
    valid = ~np.isnan(oof_p) & ~np.isnan(oof_r_lgbm) & ~np.isnan(oof_r_xgb) & ~np.isnan(oof_r_cat) & ~np.isnan(oof_r_gru)
    if valid.sum()==0:
        return {'sharpe_p':0.0,'sharpe_r':0.0,'sharpe_mult':0.0,'sharpe_stack':0.0,'best_fusion':'mult_rank','best_sharpe':0.0}, \
    ...
    df_eval = pd.DataFrame({
        ...
        'r_lgbm': oof_r_lgbm[valid], 'r_xgb': oof_r_xgb[valid], 'r_cat': oof_r_cat[valid], 'r_gru': oof_r_gru[valid]
    })

    # Stacking元模型
    X_meta_rank = df_eval[['r_lgbm','r_xgb','r_cat','r_gru']].values
    ...
    oof = {'p': oof_p, 'r_lgbm': oof_r_lgbm, 'r_gru': oof_r_gru, 'y': y, 'd': d}
...
def fit_models_and_save(input_dir: str,
    ...
    for L in (1,2,3,4):
        ...
        per_lag[L] = {
            ...
            "gru_models": artifacts.get("gru_models", []),
            "lookback_window": artifacts.get("lookback_window", 30),
            "ranker_stacker": artifacts.get("ranker_stacker", None),
...
class OnlinePredictor:
    ...
    def predict_one_batch(self, test_row_pl: pl.DataFrame) -> pd.DataFrame:
        ...
        for tgt in feats_cur['target'].unique():
            ...
            if info.get("gru_models") and info.get("lookback_window"):
                try:
                    LKB = int(info.get("lookback_window", 30))
                    sel = info["features"]
                    # 需要为该 target 构造长度为 LKB 的序列窗口
                    # 找到历史窗口：从 self.prices_hist 中取最近 LKB 天，并通过 fe.transform 生成与线上一致的特征
                    recent_hist = self.prices_hist.copy()
                    if len(recent_hist) >= LKB:
                        # 只取最近 LKB 天
                        recent_hist_win = recent_hist.iloc[-LKB:].copy()
                        # 生成该窗口的长表特征
                        feats_win = fe.transform(recent_hist_win, pairs_df)
                        feats_win = feats_win[feats_win['target'] == tgt].sort_values('date_id')
                        # 保证列齐全
                        seq_df = feats_win.reindex(columns=['date_id','target'] + sel)
                        seq_df = seq_df.fillna(0.0)
                        if len(seq_df) >= LKB:
                            seq_X = seq_df[sel].to_numpy(dtype=np.float32)[-LKB:]
                            seq_X = np.expand_dims(seq_X, axis=0)
                            gru_preds = [m.predict(seq_X, verbose=0).flatten()[0] for m in info["gru_models"]]
                            gru_pred = float(np.mean(gru_preds))
                except Exception:
                    gru_pred = 0.0

            ranker_stacker = info.get("ranker_stacker", None)
            if ranker_stacker:
                meta_features = np.array([[lgbm_pred, xgb_pred, cat_pred, gru_pred]])
                m_hat = float(ranker_stacker.predict(meta_features)[0])
            else:
                m_hat = float(np.mean([lgbm_pred, xgb_pred, cat_pred, gru_pred]))
